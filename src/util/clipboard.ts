...
export async function readClipboard(): Promise<string> {
  if (process.platform === "win32") {
    return (
      await run("powershell", ["-NoProfile", "-Command", "Get-Clipboard"])
    ).trim();
  }
  if (process.platform === "darwin") {
...
/** Open a magnet/URL with the OS default handler. Best-effort, non-blocking. */
export function openExternal(target: string): Promise<boolean> {
  return new Promise((resolve) => {
    const [cmd, args] =
      process.platform === "darwin"
        ? (["open", [target]] as [string, string[]])
        : process.platform === "win32"
          ? (["cmd", ["/c", "start", "", target]] as [string, string[]])
          : (["xdg-open", [target]] as [string, string[]]);
    try {
