...
const FEED_CACHE_KINDS: Record<DiscoveryFeed, Set<DiscoveryRequest["feedKind"]>> = {
...
function request(
  ...
  return {
    region,
    feedKind,
    ...((feedKind === "trending" || feedKind === "provider_dictionary" || feedKind === "bluray")
      ? {}
...
export function useDiscovery(
  ...
  useEffect(() => {
    ...
    void Promise.all(targets.map(async (target): Promise<DiscoveryUiSourceState> => {
      ...
      if (item.state.refresh) {
        void item.state.refresh.then((state) => {
  ...
  }, [config.discovery?.disabledSources, dateWindow, feed, revision, sourceStates]);

  const snapshots = distinctSnapshots([
